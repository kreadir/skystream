  ...
  Future<int> start() async {
    ...
    if (!kIsWeb && (defaultTargetPlatform == TargetPlatform.android || defaultTargetPlatform == TargetPlatform.iOS)) {
       ...
       while (attempts < 60) {
         ...
         // Test 2: IPv6
         try {
           final socket = await Socket.connect('::1', port, timeout: const Duration(milliseconds: 1000));
           socket.destroy();
           return port; // Success!
         } catch (_) {}
       }
       throw Exception("TorrServer failed to start: Could not establish TCP connection.");
    }
    return port;
  }
